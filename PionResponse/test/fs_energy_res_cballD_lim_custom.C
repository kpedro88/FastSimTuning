...
energyRes* get_res(int num, int imip, int ieta, bool do_fit, bool do_show, bool do_print, bool do_batch=false){
	...
	int nbins = 100;
	//custom settings to fix bad fits
	if(ieta>30 && energies[num]<=20) nbins = 50; //fewer bins for low energy in HF
	else if(ieta==49 && energies[num]>=100) { //cutoffs to avoid leakage effects for high energies in HF
		double cutoffs[] = {30,50,80,140,500,1400}; 
		Emin = cutoffs[num-10];
	}
	else if(ieta>5 && ieta<31 && energies[num]==2 && imip==0){ //fix low-energy mip fits
		nbins = 50;
		//if(ieta==11 || ieta==12 || ieta==13 || ieta==14 || ieta==18 || ieta==20 || ieta==25 || ieta==26 || ieta==27 || ieta==28 || ieta==29)
		if(ieta==17) Emax = 3;
		else Emax = 5;
		if(ieta==29) Emin = 0.3;
	}
	else if (ieta<31 && energies[num]==3000 && imip==0){ //fix very high-energy fits
		Emin = 1000;
		Emax = 4000;
		if(ieta==13 || ieta==14) nbins = 50;
	}
	else if(ieta>10 && ieta<31 && energies[num]==3 && imip==0){ //fix low-energy mip fits
		if(ieta==30) Emin = 0.15;
		else Emin = mipcut;
		
		if(ieta==15) Emax = 7;
	}
	else if(ieta>8 && ieta<31 && energies[num]==5 && imip==0){ //fix low-energy mip fits
		if(ieta==30) nbins = 50;
		Emin = 1;
	}
	else if(ieta<31 && energies[num]==2 && imip==1){ //fix low-energy nomip fits
		Emin = mipcut;
		Emax = 3;
		if(ieta==23) nbins = 75;
	}
	else if(ieta>10 && ieta<31 && energies[num]==3 && imip==1){ //fix low-energy nomip fits
		Emin = mipcut;
		if(ieta==29 || ieta==30) { Emax = 3; nbins = 50; }
		else Emax = 4;
	}
	else if(ieta==30 && energies[num]==5 && imip==1) { Emin = mipcut; Emax = 6; nbins = 50; } //single fit fix
	else if(ieta==30 && energies[num]==15 && imip==1) Emax = 20; //single fit fix
	else if(ieta==2 && energies[num]==1000 && imip==0) Emax = 1400; //single fit fix
	else if(ieta==3 && energies[num]==300 && imip==0) Emax = 450; //single fit fix
	else if(ieta==15 && energies[num]==225 && imip==0) Emax = 350; //single fit fix
	else if((ieta==4 || ieta==20) && energies[num]==5 && imip==1) Emax = 7; //single fit fix
	else if(ieta==8 && energies[num]==2 && imip==2) Emax = 3; //single fit fix
	else if((ieta==15 || ieta==30) && energies[num]==1 && imip==0) Emax = 1.5; //single fit fix
	else if((ieta==15 || ieta==17) && energies[num]==1 && imip==2) Emax = 2.5; //single fit fix
	else if(ieta==30 && energies[num]==1 && imip==2) nbins = 50; //single fit fix
	else if((ieta==29 || ieta==30) && energies[num]==2 && imip==2) Emin = 0.3; //single fit fix
	else if(energies[num]==50 && ieta==31 && imip==2) { Emax = 120; Emin = 0; nbins = 50; } //single fit fix
	...
	if (do_fit){
		...
		double par0i = ph;
		if(ieta>5 && ieta<31 && energies[num]==2 && imip==0) { //fix low-energy mip fits
			if(ieta==19 || ieta==22) par0i = ph;
			else if(ieta==29) par0i = 0.5*ph;
			else par0i = ph*0.75;
		}
		else if(ieta<31 && energies[num]==2 && imip==1){ //fix low-energy nomip fits
			if(ieta==13 || ieta ==16 || ieta==20) par0i = ph;
			else par0i = 0.75*ph;
		}
		else if(energies[num]==1 && ieta==1 && imip==0) par0i = 2*ph; //single fit fix
		else if(energies[num]==225 && ieta==15 && imip==0) par0i = 2*ph; //single fit fix
		else if(energies[num]==5 && ieta==4 && imip==1) par0i = 2*ph; //single fit fix
		else if(energies[num]==2 && ieta==8 && imip==2) par0i = 2*ph; //single fit fix
		else if(energies[num]==3000 && (ieta==3 || ieta==7 || ieta==29) && imip==0) par0i = 2*ph; //single fit fix
		else if(energies[num]==1 && (ieta==15 || ieta==17) && imip==2) par0i = 2*ph; //single fit fix
		else if(energies[num]==2 && ieta==22 && imip==2) par0i = 0.75*ph; //single fit fix;
		else if(energies[num]==3 && (ieta==16 || (ieta>=18 && ieta<=26)) && imip==2) par0i = 1.5*ph; //single fit fix
		else if(energies[num]==5 && (ieta==13 || ieta==20 || ieta==25 || ieta==28) && imip==2) par0i = 1.5*ph; //single fit fix
		else if(energies[num]==15 && ieta==50) par0i = 0.75*ph; //single fit fix
		else if(energies[num]==15 && ieta==32 && imip==2) par0i = 0.75*ph; //single fit fix
		else if(energies[num]==20 && ieta==49) par0i = 1.25*ph; //single fit fix
		else if(energies[num]==3 && ieta==28 && imip==1) par0i = 1.25*ph; //single fit fix
		gfit->SetParameters(par0i,p,s,1,1.1,1,1.1);
		...
		if(energies[num]==15 && (ieta==49 || ieta==50)) gfit->SetParLimits(1,0,100); //single fit fix
		else if(energies[num]==20 && ieta==50) gfit->SetParLimits(1,0,100); //single fit fix
...
void allthefits(string outname="tree_cballD_custom.root"){
	//energy values
	Double_t energies[] = {1., 2., 3., 5., 9., 11., 15., 20., 30., 50., 100., 150., 225., 300., 1000., 3000. };	
...
void fs_print_cfg(std::string oname="HcalResponse_new.py", std::string fname="tree_cballD_custom.root"){
	//energy values
	Double_t energies[] = {1., 2., 3., 5., 9., 11., 15., 20., 30., 50., 100., 150., 225., 300., 1000., 3000. };	
